...
scheduler._get_def_cont = function(pos) {
	...
	this._def_count.style.top = (pos.top - 200) + "px"; // 位置
...
// 隐藏
scheduler.date_picker = function(hide)
{
	var html = "<input class='dhx_readonly' type='text' readonly='true' " + (hide ? "style='display:none'" : '') + " />";
...
scheduler.form_blocks.calendar_time = {	
	render: function() {
		...
		return "<div style='height:30px;padding-top:0; font-size:inherit;' class='dhx_section_time'>" + this.date_picker(false) + "<span style='font-weight:normal; font-size:10pt;'> &nbsp;&ndash;&nbsp; </span>" + this.date_picker(true) + "</div>";
	},
	set_value: function(node, value, ev) {
		...
		// _attach_action(inputs[1], ev.end_date, 1); 必须是同一天
		_init_once = function() {};
	...
	get_value: function(node, ev) {
		...
		ev.end_date = scheduler.date.add(inputs[0]._date, selects[1].value, "minute");
