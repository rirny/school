...
scheduler.form_blocks["recurring"] = {
	...
	_init_set_value:function(node, value, ev) {
		...
		var get_rcode = {
			...
			week:function(code, dates) {
				...
				//dates._start = true;	
				var week = start_date = scheduler.date.date_part(dates.start).getDay();							
				code.push(week);
				code.splice(0, 1);
				/*
		...
		function set_repeat_code(code, dates) {
			...
			var repeat_type = code[0]=='week' && code[1]==2 ? 'week2' : code[0];
			var e = els["repeat"][({day:0, week:1, week2: 2, month:3, year:4})[repeat_type]];		
			//
					...
					el.onclick = function(){
						...
						scheduler.renderCalendar({
							...
							handler: function(new_date) {
								el.value = scheduler.templates.calendar_time(new_date);
								el._date = new Date(new_date);
	...
	button_click:function(index, el, section, cont) {
		if (!cont.open && !cont.blocked) {
			cont.style.height = "50px"; // 2013/4/27 重复框高度
...
scheduler.attachEvent("onEventCancel", function(id) {
	var ev = this.getEvent(id);
	if (typeof ev.rectyp == 'string' && ev.rec_type) {
		this._roll_back_dates(ev);
...
scheduler.__recurring_template= '<div class="dhx_form_repeat" style="height:60px;"><form><div class="dhx_repeat_left"></div><div class="dhx_repeat_divider"></div><div class="dhx_repeat_center"><div id="dhx_repeat_month">';
scheduler.__recurring_template += '<label><input class="dhx_repeat_radio" type="radio" name="repeat" value="day" />每天 </label>';
scheduler.__recurring_template += '<label><input class="dhx_repeat_radio" type="radio" name="repeat" value="week" checked />每周 </label>';
scheduler.__recurring_template += '<label><input class="dhx_repeat_radio" type="radio" name="repeat" value="week2" />每两周 </label>';
scheduler.__recurring_template += '<label><input class="dhx_repeat_radio" type="radio" name="repeat" value="month" />每月 </label>';
scheduler.__recurring_template += '</div></div> <div class="dhx_repeat_divider"></div><div class="dhx_repeat_right">';
scheduler.__recurring_template += '<label><input class="dhx_repeat_radio" type="radio" name="end" style="display:none" checked/>截止时间</label><input class="dhx_repeat_date" id="date_of_end" class="dhx_readonly" readonly="true" type="text" name="date_of_end" value="'+scheduler.config.repeat_date_of_end+'" />';
scheduler.__recurring_template += '</div> </form> </div> <div style="clear:both"></div>';